...
normalJudge = Judge {judgeId = "normal", runJudge = judge}
  where
    judge _ = Nothing

absentJudge = Judge {judgeId = "absent", runJudge = judge}
  where
    judge shogi
...
getOu board = foldl' f (Nothing, Nothing) $ cells board
  where
    f (_, w) pos@(_, Just (Piece Black _ OU)) = (Just pos, w)
...
mateJudge = Judge {judgeId = "mate", runJudge = judge}
  where
    judge shogi
      | mateTurn = Just Lose
...
checkMateJudge = Judge {judgeId = "checkMate", runJudge = judge}
  where
    judge shogi
      | mateTurn = Just Lose
      | mateNextTurn && all (pickTuple nextTurn . mate) (getNext shogi) =
        Just Win
      | otherwise = Nothing
      where
        nextTurn = turn shogi
...
tryJudge = Judge {judgeId = "try", runJudge = judge}
  where
    judge shogi
      ...
      where
        ...
        (tryTurn, tryNextTurn) =
          orderTuple
            currentTurn
            (coordBlack == Coord 5 1, coordWhite == Coord 5 9)

othelloJudge = Judge {judgeId = "othello", runJudge = judge}
  where
    judge shogi = compare . orderTuple currentTurn <$> countM (board shogi)
      where
        nextTurn = turn shogi
        currentTurn = opposite nextTurn
        compare (numTurn, numNextTurn)
          | numTurn > numNextTurn = Win
...
countM board = foldM f (0, 0) $ cells board
  where
    f acc (_, Nothing) = Nothing
    f (b, w) (_, Just (Piece Black _ _)) = Just (b + 1, w)
    f (b, w) _ = Just (b, w + 1)

gomokuJudge = Judge {judgeId = "gomoku", runJudge = judge}
  where
    judge shogi
      | winNextTurn = Just Lose
      | winTurn = Just Win
      | otherwise = Nothing
      where
        nextTurn = turn shogi
        ...
        isColor color (_, Just (Piece color' _ _))
          | color == color' = True
        isColor _ _ = False
...
winHandCountJudge = Judge {judgeId = "winHandCount", runJudge = judge}
  where
    judge shogi
