module Search.MaterialAlphaBeta
  ( alphaBeta
...
instance Game_tree CachedShogi where
  ...
  children (CachedShogi eval shogi) = map cachify' $ children shogi
    where
      cachify' shogi =
        case history shogi of
          History [] -> cachify shogi
          History (diff:_) -> CachedShogi newEval shogi
            where newEval =
                    if is_terminal shogi
                      then -evalInfinity
                      else evaluateDiff eval diff

evaluateDiff lastEval (Diff (DMove color from to kind promoted captured)) =
  -valPromote - valCapture - lastEval
  where
    valPromote =
      if promoted
        then value kind True - value kind False
        else 0
    valCapture =
      case captured of
...
evaluate shogi =
  if is_terminal shogi
    ...
    else (if turn shogi == Black
            then 1
            else -1) *
         (handValue Black - handValue White + boardValue)
  where
    handValue :: Color -> Int
    handValue color =
      sum $
      map (\(kind, num) -> value kind False * num) $ toList color $ hands shogi
    boardValue =
      sum $ do
