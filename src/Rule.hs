...
import Shogi(Judge, judgeId, Shogi(..))
import Board(AbilityProxy, abilityProxyId, Effector, effectorId, Mover, moverId, MoverPredicator, moverPredicatorId, Slicer, slicerId, Board(..))
import Shogi.Judge
...
instance Show RuleConfig where
    show ruleConfig = "RuleConfig{"
        ++ showRule (abilityProxy ruleConfig)
        ++ showRule (effector ruleConfig)
        ++ showRule (mover ruleConfig)
        ++ showRule (moverPredicator ruleConfig)
        ++ showRule (slicer ruleConfig)
        ++ showRule (judge ruleConfig)
        ++ "}"
        where showRule rule = "  "++ruleId rule++": "++ruleInstanceId rule++",\n"

injectRule :: RuleConfig -> Shogi -> Shogi
injectRule conf shogi = shogi{
        getJudge = Rule.judge conf,
        board = board'}
    where board' = (board shogi){
        getMover = mover conf,
