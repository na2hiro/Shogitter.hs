...
type NormalBoard = Board NormalMover NormalAbilityProxy NormalSlicer
...
initialBoard :: (Mover m, AbilityProxy a, Slicer s) => Board m a s
...
set :: (Coord, Cell) -> Board m a s -> Board m a s
set cp = sets [cp]
