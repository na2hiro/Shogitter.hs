...
spec :: Spec
spec = do
  describe "show" $ do it "Coord 3 5" $ show (Coord 3 5) `shouldBe` "(3,5)"
  ...
  describe "getX" $ do it "(3,5)" $ getX (Coord 3 5) `shouldBe` 3
  describe "getY" $ do it "(3,5)" $ getY (Coord 3 5) `shouldBe` 5
