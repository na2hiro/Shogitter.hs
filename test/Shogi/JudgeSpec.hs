...
import Board (Board(..), Move(..), sets)
import Board.Const (initialBoard)
import Board.Effector (underWaterEffector)
import Coord
import Hands (addToHands, initialHands)
import Piece (Color(..), Kind(..), Piece(..))
import Shogi
...
underWaterShogi =
  initialShogi {board = initialBoard {getEffector = underWaterEffector}}
...
spec = do
  describe "normalJudge" $
    let initial = initialShogi {getJudge = normalJudge}
        ...
        s' = unsafeDoMove (Move (Coord 5 9) (Coord 5 5) False) initial'
     in do it "initial" $ judge initial `shouldBe` Nothing
  ...
  describe "MateJudge" $
    let initial = initialShogi {getJudge = mateJudge}
        ...
        s'' = unsafeDoMove (Move (Coord 5 9) (Coord 5 2) False) initial
     in do it "initial" $ judge initial `shouldBe` Nothing
  ...
  describe "CheckMateJudge" $
    let initial = initialShogi {getJudge = checkMateJudge}
        moves =
          [ Move (Coord 2 7) (Coord 2 6) False
          , Move (Coord 5 1) (Coord 4 2) False
          , Move (Coord 2 6) (Coord 2 5) False
          , Move (Coord 4 2) (Coord 3 2) False
          , Move (Coord 2 5) (Coord 2 4) False
          , Move (Coord 8 2) (Coord 4 2) False
  ...
  describe "TryJudge" $
    let initial = initialShogi {getJudge = tryJudge}
        moves =
          [ Move (Coord 5 9) (Coord 6 8) False
          , Move (Coord 5 1) (Coord 5 9) False
          ]
        s = multipleUnsafeDoMove initial moves
        s' = unsafeDoMove (Move (Coord 6 8) (Coord 5 1) False) s
     in do it "initial" $ judge initial `shouldBe` Nothing
  ...
  describe "OthelloJudge" $
    let initial = initialShogi {getJudge = othelloJudge}
        ...
        diffs =
          [(Coord x y, fu White) | x <- [1 .. 9], y <- [2, 4, 5]] ++
          [(Coord x y, fu Black) | x <- [1 .. 9], y <- [6, 8]]
  ...
  describe "GomokuJudge" $
    let initial = initialShogi {getJudge = gomokuJudge}
        ...
        s''' = unsafeDoMove (Move (Coord 4 9) (Coord 5 3) False) s
     in do it "initial" $ judge initial `shouldBe` Just Lose
  ...
  describe "WinHandCountJudge" $
    let fu color = (Just $ Piece color False FU)
        hands =
          foldr
            (\kind hands -> addToHands White kind $ addToHands Black kind hands)
            initialHands $
          replicate 2 FU
        s = initialShogi {getJudge = winHandCountJudge, hands = hands}
        s' = unsafeDoMove (Move (Coord 8 8) (Coord 3 3) False) s
     in do it "2, 2" $ judge s `shouldBe` Nothing
           it "3" $ judge s' `shouldBe` Just Win
  describe "LoseHandCountJudge" $ it "" $ pendingWith "multiple judges"
