...
spec = do
    ...
    describe "hands"$
        let s@(Shogi _ _ _ hands) = unsafeDoMove (Move (Coord 8 8) (Coord 2 2) True) initial
            s'@(Shogi _ _ _ hands') = unsafeDoMove (Move (Coord 3 1) (Coord 2 2) True) s
            s''@(Shogi _ _ _ hands'') = unsafeDoMove (Put (Coord 4 5) KA) s' in do
