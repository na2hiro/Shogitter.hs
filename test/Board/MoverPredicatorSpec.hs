...
import Shogi(Shogi(..), getMovesFrom)
import Board
import Coord(Coord(..))
import Color(Color(..))
...
makeShogi :: Color -> Board m e a s mp -> Shogi m e a s mp
makeShogi turn board@Board{} = Shogi turn board initialHands

makeShogiB = makeShogi Black
makeShogiW = makeShogi White
...
spec = do
    describe "Freeze canMove"$
        ...
        it "(3,3) Shogi"$ getMovesFrom (makeShogiW s) (Coord 3 3) `shouldBe` []
        it "(4,3)"$ canMoveCoord s (Coord 4 3) `shouldBe` True
        it "(4,3) Shogi"$ getMovesFrom (makeShogiW s) (Coord 4 3) `shouldNotBe` []
        let s' = pseudoDoMove (Move (Coord 4 3) (Coord 4 4) False) s
            ...
            s''' = pseudoDoMove (Move (Coord 8 2) (Coord 4 2) False) s'' in do
                ...
                it "(4,4) Shogi"$ getMovesFrom (makeShogiW s''') (Coord 4 4) `shouldBe` []
                it "(4,7)"$ canMoveCoord s''' (Coord 4 7) `shouldBe` True
                it "(4,7) Shogi"$ getMovesFrom (makeShogiB s''') (Coord 4 7) `shouldNotBe` []
    ...
    describe "Madras canMove"$
        ...
        it "(3,3) Shogi"$ getMovesFrom (makeShogiW s) (Coord 3 3) `shouldNotBe` []
        it "(4,3)"$ canMoveCoord s (Coord 4 3) `shouldBe` True
        it "(4,3) Shogi"$ getMovesFrom (makeShogiW s) (Coord 4 3) `shouldNotBe` []
        it "(8,8)"$ canMoveCoord s2 (Coord 8 8) `shouldBe` False
        it "(8,8) Shogi"$ getMovesFrom (makeShogiB s2) (Coord 8 8) `shouldBe` []
        let s' = pseudoDoMove (Move (Coord 7 3) (Coord 7 4) False) s
            ...
            s2'' = pseudoDoMove (Move (Coord 8 8) (Coord 3 3) True) s' in do
                ...
                it "(2,2) Shogi"$ getMovesFrom (makeShogiW s'') (Coord 2 2) `shouldBe` []
                it "(2,2) Promoted"$ canMoveCoord s2'' (Coord 2 2) `shouldBe` True
                it "(2,2) Shogi"$ getMovesFrom (makeShogiW s2'') (Coord 2 2) `shouldNotBe` []
