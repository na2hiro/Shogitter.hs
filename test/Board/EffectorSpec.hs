...
import Board (Board(..), effect, sets)
...
spec = do
  describe "Othello effect" $
    let initial = initialBoard {getEffector = othelloEffector}
        diffs =
          map (\x -> (Coord x 8, Just $ Piece White False GI)) [3 .. 7] ++
          [(Coord 1 8, Just $ Piece White False HI)]
        b = sets initial diffs
        b' =
          sets b $ map (\x -> (Coord x 8, Just $ Piece Black False GI)) [3 .. 7]
        b'' =
          sets b $ map (\x -> (Coord x 8, Just $ Piece Black False GI)) [4 .. 6]
        b2 = sets b [(Coord 1 8, Just $ Piece Black False HI)]
     in do it "(8,8)" $ effect from (Coord 8 8) b `shouldBe` b'
  ...
  describe "Go effect" $
    let initial = initialBoard {getEffector = goEffector}
        diffs =
          map (\x -> (Coord x 8, Just $ Piece White False GI)) [3 .. 7] ++
          [(Coord 1 8, Just $ Piece White False HI)]
        b = sets initial diffs
  ...
  describe "Nip effect" $
    let initial = initialBoard {getEffector = nipEffector}
        diffs =
          map (\x -> (Coord x 8, Just $ Piece White False GI)) [3 .. 7] ++
          [(Coord 1 8, Just $ Piece White False HI)]
        b = sets initial diffs
  ...
  describe "UnderWater effect" $
    let initial = initialBoard {getEffector = underWaterEffector}
        piece kind = Just $ Piece Black False kind
        fu = piece FU
        diffs = [(Coord 7 6, fu), (Coord 7 7, Nothing)]
        b = sets initial diffs
        b' = sets b [(Coord 7 6, Nothing)]
        b2 =
          sets b [(Coord 7 6, Nothing), (Coord 2 7, Nothing), (Coord 2 6, fu)]
        b2' =
          sets b2 $
          (Coord 2 6, Nothing) : map (\x -> (Coord x 7, Nothing)) [3 .. 7]
        b3 = sets b2' [(Coord 2 8, Nothing), (Coord 2 7, piece HI)]
     in do it "7 6 FU" $ effect (Coord 7 7) (Coord 7 6) b `shouldBe` b'
  ...
  describe "Nuclear effect" $
    let initial = initialBoard {getEffector = nuclearEffector}
        piece kind = Just $ Piece Black False kind
        fu = piece FU
        ...
        b2 =
          sets b [(Coord 8 8, Nothing), (Coord 3 3, Just $ Piece Black True KA)]
        b2' =
          sets b2 $
          (Coord 2 2, Just $ Piece White True KA) :
          map (\x -> (Coord x 3, Just $ Piece White True FU)) [2, 4]
     in do it "(7,4)" $ effect from (Coord 7 4) b `shouldBe` b'
  ...
  describe "Donden effect" $
    let initial = initialBoard {getEffector = dondenEffector}
        piece kind = Just $ Piece Black False kind
        fu = piece FU
        ...
        b' =
          sets
            b
            [ (Coord 7 2, Just $ Piece Black False GI)
            , (Coord 7 1, Just $ Piece White False FU)
            ]
        diffs2 = [(Coord 7 2, Just $ Piece Black True FU), (Coord 7 7, Nothing)]
        b2 = sets initial diffs2
        b2' =
          sets
            b2
            [ (Coord 7 2, Just $ Piece Black False GI)
            , (Coord 7 1, Just $ Piece White True FU)
            ]
     in do it "(7,2)" $ effect from (Coord 7 2) b `shouldBe` b'
  ...
  describe "Gravity effect" $
    let piece kind = Just $ Piece Black False kind
        fu = piece FU
        initial = effect from from initialBoard {getEffector = gravityEffector}
        diffs = [(Coord 7 6, fu), (Coord 7 7, Nothing)]
        b = sets initial diffs
        b' =
          sets
        ...
        b2 =
          sets
            initial
            [(Coord 2 2, Nothing), (Coord 8 2, Just $ Piece White False HI)]
     in do it "7 6 FU" $ effect from (Coord 7 6) b `shouldBe` b'
