...
spec = do
  ...
  describe "Donut destinationsAt" $
    let diffs =
          [ (Coord 9 7, Nothing)
          , (Coord 7 9, Nothing)
          , (Coord 1 5, Just $ Piece White False HI)
          ]
        b = sets initialBoard {getSlicer = donutSlicer} diffs
     in do it "(8,8)" $
             destinationsAt b (Coord 8 8) `shouldMatchList`
             [ Coord 9 7
             , Coord 1 6
  ...
  describe "Reflect destinationsAt" $
    let diffs =
        ...
        b = sets initialBoard {getSlicer = reflectSlicer} diffs
     in do it "(8,8)" $
             destinationsAt b (Coord 8 8) `shouldMatchList`
             [ Coord 9 7
             , Coord 8 6
